...
export const Task4 = () => {
    ...
    const answerHandler = async (data: TaskFormValues) => {
        if (data.answer.trim().toLocaleLowerCase() === Answers.TASK_4) {
            playSuccess();
            await dialogs.alert(
                `Да, ${Answers.TASK_4}, верно! Жми, чтобы узнать точку на карте с координатами кода к следующему заданию.`,
                {
                    title: 'Ответ верный',
            ...
            >
                <Stack direction="column" gap={3}>
                    ...
                    <Alert
                        ...
                        sx={{ textAlign: 'center' }}
                    >
                        Как называется животное, которое одновременно
                        <br /> имеет признаки кота и собаки?
                    </Alert>
                    <TextFieldElement
                        name="answer"
                        placeholder={'введи название животного'}
                        control={control}
                        required
                    />
                    <Button
