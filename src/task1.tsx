...
export const Task1 = () => {
    ...
    const answerHandler = async (data: TaskFormValues) => {
        if (data.answer.trim().toLocaleLowerCase() === Answers.TASK_1) {
            playSuccess();
            await dialogs.alert(
                `Да, ${Answers.TASK_1}, верно! Жми, чтобы узнать точку на карте с координатами кода к следующему заданию.`,
                {
                    title: 'Ответ верный',
            ...
            >
                <Stack direction="column" gap={3}>
                    ...
                    <Alert
                        ...
                        sx={{ textAlign: 'center' }}
                    >
                        Какой твой знак зодика?
                    </Alert>
                    <TextFieldElement
                        name="answer"
                        placeholder={'введи знак зодика'}
                        control={control}
                        required
                    />
                    <Button
